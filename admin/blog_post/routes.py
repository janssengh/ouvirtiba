...
def excluir_arquivo_fisico(caminho_relativo):
    ...
    try:
        ...
        if os.path.exists(caminho_completo):
            ...
            logging.info(f"Arquivo excluído: {caminho_relativo}")
            return True
        else:
            logging.warning(f"Arquivo não encontrado: {caminho_relativo}")
            return False
...
def limpar_imagens_orfas(imagens_antigas, imagens_novas):
    ...
    if imagens_para_excluir:
        logging.info(f"{len(imagens_para_excluir)} imagem(ns) órfã(s) excluída(s)")
...
def blog_delete(post_id):
    """Exclui um post do blog e suas imagens associadas."""
    ...
    try:
        ...
        logging.info(f"Post '{post_title}' (ID: {post_id}) excluído com sucesso")
        flash(f'Post "{post_title}" excluído com sucesso!', 'success')

    except Exception as e:
        db.session.rollback()
        logging.error(f"Erro ao excluir post {post_id}: {e}")
