from sqlalchemy.exc import IntegrityError # Importe isso no topo do arquivo
from flask import Blueprint, render_template, session, flash, url_for, redirect, request, current_app, jsonify
from .models import Product, Store, User, Brand, Category, Size, Packaging, Color
from admin.client.models import Client
from .forms import LoginFormulario, RegistrationForm, ProductForm, StoreForm
...
import os, secrets
import requests # Necessário para a API ViaCEP
from werkzeug.utils import secure_filename # Para o nome seguro do arquivo
...
def save_store_image(form_file, old_filename=None):
    """
    Salva uma nova imagem na pasta /static/img/admin.
    - Exclui a imagem anterior (se informada).
    - Retorna o nome do novo arquivo salvo.
    """
    if not form_file or form_file.filename == '':
        return None

    # Gera nome aleatório preservando a extensão
    random_hex = secrets.token_hex(8)
    _, file_ext = os.path.splitext(form_file.filename)
    filename = f"{random_hex}{file_ext.lower()}"

    # Define diretório completo
    ...
    # Exclui imagem anterior (se houver e se for diferente)
    if old_filename:
        old_path = os.path.join(img_dir, old_filename)
        if os.path.exists(old_path) and old_filename != filename:
            try:
                os.remove(old_path)
                print(f"Imagem antiga removida: {old_filename}")
...
def produto_editar(id):
    ...
    return render_template(
        ...
        tipo_nome=tipo_nome,
        tipo_produto=type_id,
        titulo=f'Editar: {produto.name}'
    )

@admin_bp.route('/produto/inserir', methods=['GET', 'POST'], defaults={'type_id': None})
...
def produto_inserir(type_id):
    ...
    if form.validate_on_submit():
        try:
            ...
            color_id = int(request.form.get('cor').split(',')[0])
            size_id = int(request.form.get('tamanho'))
            packaging_id = int(request.form.get('embalagem'))
            
            # Tratamento da Cor
            color_name = "Não informada"
            if color_id:
                cor_obj = Color.query.get(color_id)
                if cor_obj:
                    color_name = cor_obj.name
            ...
            def salvar_imagem(campo_nome):
                ...
                if arquivo and arquivo.filename:
                    nome_seguro = secrets.token_hex(10) + os.path.splitext(arquivo.filename)[1]
                    arquivo.save(os.path.join(upload_dir, nome_seguro))
                    return nome_seguro
                return 'image.jpg'
            ...
            db.session.rollback()
            # Erro comum: violação da constraint UNIQUE (produto já cadastrado com essas características)
            flash('Erro: Já existe um produto cadastrado com este nome para esta marca/categoria/cor/tamanho.', 'danger')
            print(f"❌ Erro de Integridade: {str(e)}")
            
        except Exception as e:
            db.session.rollback()
            # Captura qualquer outro erro (banco fora do ar, erro de permissão de pasta, etc)
            flash(f'Ocorreu um erro inesperado ao salvar: {str(e)}', 'danger')
            print(f"❌ Erro Geral: {str(e)}")

    elif request.method == 'POST':
        # Se o formulário nem passar na validação do WTForms
        flash(f'Verifique os campos do formulário: {str(form.errors)}', 'warning')
        print("⚠️ Erros de validação:", form.errors)

    return render_template(
        'admin/product_ins.html',
        form=form, marcas=marcas, categorias=categorias, cores=cores,
        tamanhos=tamanhos, embalagens=embalagens, titulo='Cadastrar Produto',
        type_id=type_id, tipo_nome=tipo_nome
    )
...
# No routes.py, após o bloco de PACKAGING (Embalagens)

# ==================== STORE (LOJAS) ====================
@admin_bp.route('/store/list')
@login_required
def store_list():
    if 'email' not in session:
        flash('Favor fazer login primeiro!', 'danger')
    ...
    # Para a listagem de lojas, não filtra por store_id, lista todas
    stores = Store.query.order_by(Store.name.asc()).all()

    return render_template('admin/store_list.html', stores=stores, titulo='Lojas Cadastradas')

# Busca CEP
@admin_bp.route('/api/busca_cep/<cep>')
def busca_cep(cep):
    ...
    try:
        ...
        # Retorna apenas os campos relevantes para o formulário
        return jsonify({
            'address': data.get('logradouro'),
            'neighborhood': data.get('bairro'),
            'city': data.get('localidade'),
            'region': data.get('uf'), # UF é a região (estado)
            'complement': data.get('complemento')
        }), 200

    except requests.exceptions.RequestException:
        # Erro de conexão
        return jsonify({'erro': 'Erro de conexão com a API ViaCEP'}), 500
    except Exception as e:
        # Outros erros
        return jsonify({'erro': f'Erro inesperado: {e}'}), 500

@admin_bp.route('/store/ins', methods=['GET', 'POST'])
@login_required
def store_ins():
    if 'email' not in session:
        flash('Favor fazer login primeiro!', 'danger')
    ...
    if form.validate_on_submit():
        ...
        try:
            ...
            def save_logo(file):
                """Salva imagem em /static/img/admin com nome hexadecimal."""
                if not file or file.filename == '':
                    return None
                random_hex = secrets.token_hex(8)
                _, ext = os.path.splitext(file.filename)
                filename = f"{random_hex}{ext.lower()}"
                
                # Caminho atualizado para /static/img/admin
                img_dir = os.path.join(current_app.root_path, 'static', 'img', 'admin')
                os.makedirs(img_dir, exist_ok=True)
            ...
            # === 2. Salvar logotipos ===
            logo_name = save_logo(form.logotipo_1.data)
            logo_white_name = save_logo(form.logotipo_2.data)

            # === 3. Campo home (checkbox) ===
            home_status = 'S' if form.home.data else 'N'

            if not form.freight_rate.data:
                form.freight_rate.data = 0

            if not form.pages.data:
                form.pages.data = 0
            ...
            nova_loja = Store(
                ...
                logo=logo_name if logo_name else 'default_logo.png',
                url=form.url.data,
                code=form.code.data,
            ...
            db.session.commit()
            flash(f'A loja "{nova_loja.name}" foi cadastrada com sucesso!', 'success')
...
@admin_bp.route('/store/upd/<int:store_id>', methods=['GET', 'POST'])
@login_required
def store_upd(store_id):
    if 'email' not in session:
        flash('Favor fazer login primeiro!', 'danger')
        return redirect(url_for('auth.login', origin='admin'))

    store = Store.query.get_or_404(store_id)
    form = StoreForm(obj=store)  # pega os dados do objeto
    ...
    if request.method == 'POST' and form.validate_on_submit():
        ...
        store.code = form.code.data  # CNPJ
        store.url = form.url.data
        store.zipcode = form.zipcode.data
        store.address = form.address.data
        store.number = form.number.data
        store.complement = form.complement.data
        store.neighborhood = form.neighborhood.data
        ...
        # --- LOGOTIPO 1 (logo) ---
        # ==========================================================
        if form.logotipo_1.data and hasattr(form.logotipo_1.data, 'filename') and form.logotipo_1.data.filename:
            new_logo_1_fn = save_store_image(form.logotipo_1.data, old_filename=store.logo)
            if new_logo_1_fn:
                store.logo = new_logo_1_fn
                campos_alterados = True

        # ==========================================================
        # --- LOGOTIPO 2 (logo_white) ---
        # ==========================================================
        if form.logotipo_2.data and hasattr(form.logotipo_2.data, 'filename') and form.logotipo_2.data.filename:
            new_logo_2_fn = save_store_image(form.logotipo_2.data, old_filename=store.logo_white)
            if new_logo_2_fn:
                store.logo_white = new_logo_2_fn
                campos_alterados = True
        ...
        if campos_alterados:
            try:
                db.session.commit()
                flash(f'A loja "{store.name}" foi atualizada com sucesso!', 'success')
                return redirect(url_for('admin.store_list'))
            except Exception as e:
                db.session.rollback()
                flash(f'Erro ao atualizar a loja: {e}', 'danger')
        else:
            flash('Nenhuma alteração válida foi detectada.', 'info')
    ...
    return render_template('admin/store_upd.html',
                           form=form,
                           store=store,
                           titulo=f'Editar Loja: {store.name}')

@admin_bp.route('/store/del/<int:store_id>', methods=['POST'])
@login_required
def store_del(store_id):
    if 'email' not in session:
        flash('Favor fazer login primeiro!', 'danger')
        return redirect(url_for('auth.login', origin='admin'))
...
def user_list():
    ...
    # Lista todos os usuários (pode ser ajustado para filtrar por loja, se necessário)
    users = User.query.order_by(User.name.asc()).all()

    return render_template('admin/user_list.html', users=users, titulo='Usuários Cadastrados')


@admin_bp.route('/user/ins', methods=['GET', 'POST'])
@login_required
def user_ins():
    if 'email' not in session:
        flash('Favor fazer login primeiro!', 'danger')
    ...
    stores = Store.query.order_by(Store.name.asc()).all() # Lista de lojas para o select
    ...
    if request.method == 'POST':
        ...
        if not all([name, username, email, password, store_id]):
            ...
            try:
                ...
                db.session.commit()
                flash(f'O Usuário "{name}" foi cadastrado com sucesso!', 'success')
                return redirect(url_for('admin.user_list'))
            except Exception as e:
                db.session.rollback()
                flash(f'Erro ao cadastrar o usuário: {e}', 'danger')
        
    return render_template('admin/user_ins.html', stores=stores, titulo='Novo Usuário')

@admin_bp.route('/user/upd/<int:user_id>', methods=['GET', 'POST'])
@login_required
def user_upd(user_id):
    ...
    return render_template('admin/user_upd.html', user=user, stores=stores, titulo=f'Editar Usuário: {user.name}')

@admin_bp.route('/user/del/<int:user_id>', methods=['POST'])
@login_required
def user_del(user_id):
    ...
    try:
        ...
        db.session.commit()
        flash(f'O Usuário "{nome_usuario}" foi excluído com sucesso!', 'success')
    except Exception as e:
        db.session.rollback()
