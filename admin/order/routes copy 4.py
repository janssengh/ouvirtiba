...
def orderpdf(order_id):
    ...
    # ✅ Adiciona o número de série à descrição do produto para cada item
    for item in ordersitem:
        if item.serialnumber:
            item.product_display_name = f"{item.product.name} SN {item.serialnumber}"
        else:
            item.product_display_name = item.product.name
