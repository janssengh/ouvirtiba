...
# -------------------------------------------------
# 🟢 GERA XML ASSINADO E SALVA EM admin/nfe/output
# -------------------------------------------------

@nfe_bp.route('/admin/nfe/<int:id>/xml-signed', methods=['GET'])
def generate_xml_signed(id):
    """
    Executa o mesmo fluxo do test_nfce.py:
    Lê XML → valida → assina → valida assinatura → download
    """
    ...
    try:
        ...
        if not invoice.xml_path or not os.path.exists(invoice.xml_path):
            raise Exception(f"XML não encontrado em: {invoice.xml_path}")
        ...
        # =========================================================
        # TESTE 2 — Validação da estrutura (soft)
        # =========================================================
        required_tags = [
            'ide', 'emit', 'dest', 'det', 'total',
            'transp', 'pag', 'infAdic', 'infNFeSupl'
        ]

        for tag in required_tags:
            if root.find(f'.//nfe:{tag}', namespace) is None:
                flash(f"⚠️ Tag <{tag}> ausente no XML", "warning")
        ...
        valido, msg = validar_qrcode_url(qrcode_url)
        if not valido:
            flash(f"⚠️ QR Code inválido: {msg}", "warning")

        # =========================================================
        # TESTE 4 — Assinatura
        # =========================================================
        if not os.path.exists(CFG.CERT_PFX_PATH):
            raise Exception("Certificado digital não encontrado")

        xml_assinado = assinar_xml_nfce(
            xml_content,
            CFG.CERT_PFX_PATH,
            CFG.CERT_PFX_PASSWORD
        )
        ...
        if not valido:
            raise Exception(f"Assinatura inválida: {mensagem}")

        # =========================================================
        # TESTE 6 — Retorno para download
        # =========================================================
        filename = f"NFCe_{invoice.number}_assinado.xml"

        return send_file(
            BytesIO(xml_assinado),
        ...
        flash(f"❌ Erro ao gerar XML assinado (debug): {e}", "danger")
        return redirect(url_for('nfe_bp.nfe_list'))
